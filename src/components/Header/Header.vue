  ...
  <v-app-bar class="main-header" height="64" fixed color="primary" dark>
    <v-btn icon class="mx-1" @click.stop="TOGGLE_DRAWER">
      <template v-if="DRAWER_STATE"
        ><v-icon style="font-size: 28px">mdi-arrow-left</v-icon></template
      >
      <template v-else
        ><v-icon style="font-size: 28px">mdi-menu</v-icon></template
      >
    </v-btn>
    <!-- /**BUTTON TOGGLE_DRAWER */ -->
    <v-toolbar-title>Administration-Management</v-toolbar-title>
    <!-- /**TITLE TOOLBAR */ -->
    <v-spacer></v-spacer>

    <!-- /**BUTTON UNLOCK NEW VERSION */ -->
    <v-btn
    ...
    <!-- /**BUTTON AND PLACEHOLDER SEARCHING */ -->
    <Search />
      ...
      <template v-slot:activator="{ on, attrs }">
        ...
        >
          <v-badge :value="messageBadge" color="warning" content="3" overlap>
            <v-icon
              :color="colorConfig.light.iconColor"
              style="font-size: 28px"
            >
    ...
    <v-menu min-width="180" offset-y bottom left nudge-bottom="10">
      <template v-slot:activator="{ on, attrs }">
        <v-btn class="mr-0" icon v-bind="attrs" v-on="on">
          <v-icon style="font-size: 28px" :color="colorConfig.light.iconColor"
            >mdi-account</v-icon
          >
        </v-btn>
      </template>
